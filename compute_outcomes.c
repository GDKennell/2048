...
void compute_outcomes(uint64_t *allBoards,unsigned int layer_num)
{
    size_t MAX_BLOCK_SIZE = 500000;
    init_output_buffer(MAX_BLOCK_SIZE * 30);

    char *filepath = "./compute_outcomes.gpu64.bc";
    char *function_name = "compute_outcomes";
         ...
         orig_start_index += MAX_BLOCK_SIZE)
    {
        ...
        void *input_buffers[] =             {allBoards + orig_start_index, NULL};
        size_t input_buffer_sizes[] =       {this_block_size * sizeof(uint64_t)};
        size_t input_buffer_max_sizes[] =   {MAX_BLOCK_SIZE * sizeof(uint64_t) };

        size_t outputBufferSize = 30 * this_block_size * sizeof(uint64_t);
        size_t outputBufferMaxSize = 30 *  MAX_BLOCK_SIZE * sizeof(uint64_t);
        ...
        uint64_t result_layer_offset = layer_start + 30 * (orig_start_index - prev_layer_start);
        for (uint64_t i = 0; i < 30 * this_block_size; ++i)
        {
